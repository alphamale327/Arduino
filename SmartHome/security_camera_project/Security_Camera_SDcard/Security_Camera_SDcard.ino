...
#define light 3
int incomingByte; 
SoftwareSerial xbeeConnection = SoftwareSerial(6, 7);
...
void loop() {
  if (xbeeConnection.available() > 0) {
    // read the oldest byte in the serial buffer:
    incomingByte = xbeeConnection.read();
...
void takingPic(){
  ...
  //enable to save up to 100 images.
   char filename[13];
...
void pictureTransmission(char filename[13]){
  ...
  if (myFile) {
    ...
    while (myFile.position() < myFile.size()) {
        xbeeConnection.write(myFile.read());
    }
    // close the file:
    myFile.close();
    //delay(6000); //time taking for processing-2
    SD.remove(filename);
    Serial.println("done!");
  }else{
